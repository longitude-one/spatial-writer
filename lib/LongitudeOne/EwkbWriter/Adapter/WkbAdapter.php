...
class WkbAdapter implements AdapterInterface
{
     ...
     *
     * @throws UnsupportedSpatialType      when the spatial type is not supported
     * @throws UnsupportedSpatialInterface when the spatial interface is not supported
     */
    ...
    private function writeCoordinates(SpatialInterface $spatial): string
    {
        return match (true) {
            ...
            default => throw new UnsupportedSpatialInterface($spatial::class),
        };
    }
    ...
    private function writeType(SpatialInterface $spatial): string
    {
        return match ($spatial->getType()) {
            ...
            SpatialInterface::GEOMETRYCOLLECTION => pack('L', 7),
            default => throw new UnsupportedSpatialType(sprintf('WKB adapter does not support spatial type %s', $spatial->getType()))
        };
    }
}
